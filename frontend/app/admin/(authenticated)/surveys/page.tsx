...
import { Plus, Settings, CheckCircle2, MessageSquare, AlertCircle, History, Eye, X, Star, Calendar, User, Mail } from "lucide-react";
...
export default function SurveysPage() {
  ...
  useEffect(() => {
    ...
    if (activeTab === "surveys") {
      satisfactionAdminService.getAll(session.backendJwt).then(data => {
        setSurveys(data);
        setLoading(false);
      }).catch(err => {
        console.error(err);
      ...
      satisfactionAdminService.getAllSubmissions(session.backendJwt).then(data => {
        setSubmissions(data);
        setLoading(false);
      }).catch(err => {
        console.error(err);
    ...
    <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto">
            ...
            <table className="w-full text-left text-sm text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-slate-500">No hay encuestas configuradas.</td>
                  </tr>
                ) : (
                  surveys.map((survey) => (
            ...
            <table className="w-full text-left text-sm text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-slate-500">Aún no se han recibido respuestas.</td>
                  </tr>
                ) : (
                  submissions.map((sub) => (
      ...
      {selectedSubmission && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full shadow-2xl border border-slate-200 p-6 max-h-[85vh] overflow-y-auto flex flex-col gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <h4 className="text-sm font-bold uppercase tracking-wider text-purple-600 bg-purple-50 px-3 py-1.5 rounded-lg">
                  1. Calificación a tu PathMentor
                </h4>
